...
**Status:** design doc + **implemented M1 & Lean v1**, on **either engine** — the sanctioned plain-Docker variant (default; see [`M1.md`](./M1.md) and [`v1.md`](./v1.md)) *or* the headline **Coolify** engine (see [`coolify/README.md`](./coolify/README.md)), selected by `KIOSK_DEPLOY_BACKEND`; set up local or remote with [`SETUP.md`](./SETUP.md) · **Trust model:** trusted-internal, accident-hardened · **Purpose:** alignment, stakeholder buy-in, discovery.
