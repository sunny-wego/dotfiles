...
norm="$(
  SELECT_FILE="$SELECT_FILE" SCOUT_FILE="$SCOUT_FILE" TESTS_FILE="$TESTS_FILE" python3 <<'PY'
import os, json
...
out = {
    ...
    "selected_note": f"Scout kept {ran_n} of {len(units)}" if units else "",
    "units": units,
    "tests": tests_str,
