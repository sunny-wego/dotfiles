...
3. **Stage 0 — Scout** — a cheap `zeus:scout` triage that decides how much of the below to run and at which model tier, and sizes the Stage-2 fan-out; it *refines* `requires-review.sh`'s derived gating, never lowers it. See below.
...
- **`claim_inventory` sizes the Stage-2 fan-out** — how many `zeus:diagnostician` grounding agents spawn and where they point — and pre-flags suspicious cites for the citation-drift check. This replaces the old ad-hoc "scoped to ROI" guess with an explicit list.
...
1. Spawn a **`zeus:cold-reader`** (Task tool) with *only* the rendered body — the agent has no repo or conversation access by construction — role-played as **a skeptical senior stakeholder with 10 minutes**.
2. Require four outputs:
   ...
   - **Discussability audit**: is each question genuinely open or rhetorical? does every option carry a real tradeoff, or is one a strawman propping up the lean? can a reviewer disagree with a single quotable row? *what comment would you actually post?* Score the 8 criteria in `quality-criteria.md` here, from the outside.
   - **Audience-fit check** (criterion 8 — conditional): decide from the body who must sign off. **If the audience reaches beyond the implementing team** (a cross-team RFC, a `cc @non-eng`, a call a PM / lead / sponsor ratifies) **and** the domain is jargon-heavy, re-read the visible layer *as a non-technical stakeholder* and answer: could you state what's being decided and why it matters without opening code or a spec? Flag any decision-load-bearing term used before it's defined, and whether a skippable plain-language layer (primer + glossary) is present when it's needed. **If the doc is engineer-only or mechanical, state that in one line and skip this** — a missing primer there is correct right-sizing, not a gap (don't manufacture one).
...
The cold reader takes every empirical claim on faith — faith is where fluent hallucinations live. A refutation-framed pass sized by the scout's `claim_inventory` and **fanned out** as read-only `zeus:diagnostician` subagents — one per lean-supporting claim and one for the spot-check batch, all in a single turn — each **returning** what it could refute from the pinned source vs. what it couldn't. The checks are independent reads of different parts of the world, so the merged verdict set is identical to a serial pass at a fraction of the wall-clock. **Executing** fenced artifacts (SQL/code) stays in the orchestrator, not a subagent: a `zeus:diagnostician` is read-only (no Bash), and execution is also where the DB-branch authorization lives (see below), so the main context runs each artifact somewhere the author didn't seed. **Tier per check** (`tier_per_check`): citation-drift and spot-check-bait are mechanical grep-and-read against the pinned blob → **Haiku/Sonnet 5**; lean-supporting-claim refutation is judgment → **Opus** (per-call model override). The **merge + verdict synthesis stays Opus in the main context** (`review.synthesis_model`) — cheap models generate candidates, Opus decides. Only this pass promotes a finding to Confirmed. Sequencing rule: on a **first** render, run Stage 1 before Stage 2 (a BLOCKED verdict forces state edits that would waste grounding work); on a **re-render after fixes**, Stage 1 and the touched-claim Stage 2 re-checks may launch together — both gates pass independently, so ordering can't change the outcome, only the latency.
...
For contested or high-stakes decisions only (the scout gates this — `stages.steelman`). One `zeus:cold-reader` per *anticipated objector* (objectors are fewer and more concrete than rejected alternatives), on **Opus** (a steelman is judgment, not a mechanical check — the cold-reader's default tier), writes the strongest comment that person would post *from the proposal as written* — a concrete losing scenario, not rhetoric. Spawn them in a single turn: objections are independent by construction; A's steelman never depends on B's. A steelman is a **judgment-layer** output — post it as a named open question, never stamp it Confirmed. Amend to pre-answer each, or post it yourself as a named open question. Arriving with the opposition's best argument already addressed is the cheapest buy-in accelerator there is.
