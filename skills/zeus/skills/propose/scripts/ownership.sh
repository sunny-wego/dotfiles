...
# a teammate-filed issue doesn't make its body yours to rewrite.
#
# Fail-safe: if either login can't be determined (lookup error, no auth), we report
# mine:false + determined:false so the caller REFUSES the amend rather than risking
# a clobber. Always exits 0 — the caller decides what to do with the verdict.
#
# Usage:  ownership.sh <number> [--repo <owner/name>]
# Output: {"number":N,"author":"…","viewer":"…","mine":bool,"determined":bool}
...
[ -n "$number" ] || { echo "usage: ownership.sh <number> [--repo owner/name]" >&2; exit 2; }
...
jq -nc --argjson n "$number" --arg a "$author" --arg v "$viewer" \
